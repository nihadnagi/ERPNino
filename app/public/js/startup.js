...
erpnext.startup.show_expiry_banner = function() {
	...
	if (0 <= diff && diff <= 10) {
		...
		msg = repl(wn._('Your ERPNino subscription will')+' <b>expire %(expiry_string)s</b>. %(payment_link)s',
			{ expiry_string: expiry_string, payment_link: payment_link });
	} else if (diff < 0) {
		msg = repl(wn._('This ERPNino subscription')+'<b>'+wn._('has expired')+'</b>. %(payment_link)s', {payment_link: payment_link});
	}
